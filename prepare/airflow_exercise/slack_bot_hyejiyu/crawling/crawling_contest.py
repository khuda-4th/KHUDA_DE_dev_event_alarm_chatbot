    ...
    async with session.get(url) as response:
        ...
        for contest in contests:
            try:
                ...
                print(f'Crawl Success: {title}  [{link}]')
...
def contest_crawling():
    ...
    async def main():
        ...
        # CSV 파일 생성
        with open(f'/home/ubuntu/airflow/airflow/data/contest_{date}.csv', 'w', newline='', encoding='utf-8') as file:
            csv_writer = csv.writer(file)
            # CSV 파일 헤더 작성
            csv_writer.writerow(['제목', '카테고리', '주최', '대상', '접수 시작일',
                                '접수 마감일', '심사 시작일', '심사 종료일', '심사 마감일', 'D-Day', '상태', '링크', '이미지 링크'])
