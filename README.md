...
>- Airflow는 데이터 파이프라인을 스케줄링 및 관리하기 위한 오픈소스 플랫폼이다. Airflow DAG은 작업이 어떻게 연결되고 실행되는 지를 정의한다.
>- **AWS S3(Amazon S3)**: Amazon Web Services (AWS)에서 제공하는 클라우드 기반의 객체 스토리지 서비스입니다. Amazon S3를 사용하면 데이터를 안전하게 저장하고 웹을 통해 언제 어디서나 이 데이터에 액세스할 수 있습니다. 아래는 Amazon S3의 주요 특징과 사용 사례입니다:
>- 각각의 **dag.py**에서 크롤링을 자동화하고 csv 파일로 저장한다.
